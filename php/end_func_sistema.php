...
if ($botao == "incluir") 
{
	...
	try{
	    $Comando=$conexao->prepare("insert into endereco_func(cep, estado, cidade, rua, numero, bairro, complemeto)values(?,?,?,?,?,?,?);");
...
if ($botao == "deletar") {
	try{
	    $Comando=$conexao->prepare("delete from endereco_func where cep=?");
		$Comando->bindParam(1,$cep);
...
if ($botao == "consultar") {
	try{
	    $Comando=$conexao->prepare("select * from endereco_func where cep=?");
		$Comando->bindParam(1,$cep);
...
if ($botao == "alterar") {
	try{
	    $Comando=$conexao->prepare("update endereco_func set cep=? estado=? cidade=? rua=? numero=?, bairro=?, complemeto=?, where cep=?");
		$Comando->bindParam(1,$cep);
		$Comando->bindParam(2,$estado);
		$Comando->bindParam(3,$cidade);
		$Comando->bindParam(4,$rua);
